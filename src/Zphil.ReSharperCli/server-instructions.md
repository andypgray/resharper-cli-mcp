...
Before changing what ReSharper enforces — a settings file, a cleanup profile, suppressing a rule, or stopping cleanup from normalizing a deliberate style — read the `resharper://guides/configuration` resource. When a call cannot find the solution, times out, or comes back truncated, read `resharper://guides/setup`.
